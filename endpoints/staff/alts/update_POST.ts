...
export async function handle(request: Request) {
  try {
    ...
    const updatedAlt = await db.updateTable('staffAlts')
      .set(updateData)
      .where('id', '=', id)
      .returningAll()
      .executeTakeFirstOrThrow();

    return new Response(superjson.stringify({ alt: updatedAlt } satisfies OutputType));
  } catch (error) {
    console.error("Error updating staff alt:", error);
